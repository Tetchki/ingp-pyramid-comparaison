...
\paragraph{Results.}
We optimized a \(4\text{K}\) texture of \emph{The Great Wave off Kanagawa}
for 128 iterations under an $\ell_2$ reconstruction loss. The per-iteration
loss curves of the Laplacian pyramid and the linear hash grid are
indistinguishable within numerical precision, fully overlapping across
the entire optimization.  Both follow the same non-monotonic descent
pattern and reach identical minima, confirming that their gradient flows
and convergence dynamics are equivalent in practice.  The final
reconstructed textures are visually and numerically identical to machine
precision, as predicted by the analytical gradient identity.

\begin{figure}[h]
\centering
\includegraphics[width=0.82\linewidth]{loss_comparison.png}
\caption{Loss evolution during the \(4\text{K}\) texture optimization of
\emph{The Great Wave off Kanagawa} over 128 iterations.  The Laplacian
pyramid (orange) and the linear hash grid (blue) curves overlap exactly,
confirming identical optimization dynamics and loss landscapes.}
\label{fig:lossplots}
\end{figure}
